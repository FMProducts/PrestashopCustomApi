<?php
	...
	$discount_products = R::getAll($base_query."`ps_product_lang`.`id_lang` = ? AND `ps_image`.`cover` = 1 AND `ps_product`.`on_sale` = 1 ORDER BY `ps_product`.`date_upd` DESC LIMIT ? , ?", [$now , $lang, intval($offset), intval($limit)]);

	$result = array('code' => 1, 'products' => $discount_products);
